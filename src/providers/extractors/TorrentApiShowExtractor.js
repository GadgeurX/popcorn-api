...
export default class Extractor extends BaseExtractor {
  ...
  async getShow(show) {
    try {
      ...
      if (show.provider == "T411")
        var html = await torrentapiObj.getTorrentDetails(show.torrent);
      else if (show.provider == "Torrent9")
      {
        var html = show.torrent.desc;
        html = await rp(html);
      }
      const $ = cheerio.load(html);
      if (show.provider == "T411")
      {
        show.episodes[show.season][show.episode][show.quality].url = $("td.tTorrentDL.citems").eq(2).children('a').attr("href");
      }
      else if (show.provider == "Torrent9")
      {
        show.episodes[show.season][show.episode][show.quality].url = $("div.download-btn").eq(1).children('a').attr("href");
        if (show.episodes[show.season][show.episode][show.quality].url == null)
          show.episodes[show.season][show.episode][show.quality].url = show.torrent.link; 
      }
      const newShow = await this._helper.getTmdbInfo(show.showTitle);
  ...
  async search(provider) {
    try {
      const torrents = await this._contentProvider.search(provider.query, "TV", 1000000);
